...
@pytest.fixture
def app_instance():
    """Provides a GoogleDriveApp instance for tests."""
...
def test_universal_mcp_google_drive_app_initialization(app_instance):
    """
    Test that the GoogleDriveApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "google-drive", "GoogleDriveApp instance has unexpected name."


def test_universal_mcp_google_drive_tool_docstrings_format(app_instance):
    """
    Test that each tool method in GoogleDriveApp has a well-formatted docstring,
...
def test_universal_mcp_google_drive_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in GoogleDriveApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
